# example of creating nat networks and placing vms on each
#
# documentation: 
...
locals {
  seedvm = {
    "nat1" = { os_code_name = "focal", network = "nat130", prefixIP = "192.168.130", octetIP = "10", vcpu=2, memoryMB=1024*2 },
    "nat2" = { os_code_name = "focal", network = "nat131", prefixIP = "192.168.131", octetIP = "10", vcpu=2, memoryMB=1024*2 }
  }
  # use local instance of dnsmasq listening on local 'l0'
  networks = {
    "nat130" = { mode="nat", address="192.168.130.0/24", domain="home.lab", dns_forwarder="127.0.0.1"},
    "nat131" = { mode="nat", address="192.168.131.0/24", domain="home.lab", dns_forwarder="127.0.0.1"},
  }
}
